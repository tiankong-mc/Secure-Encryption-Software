    ...
    def trigger_emergency_backup(self):
        ...
        for widget in QApplication.topLevelWidgets():
            if widget.__class__.__name__=='MainWindow':
                main_window=widget;break
        if main_window is None:
            QMessageBox.critical(self,"错误","无法找到主窗口");return
        storage=main_window.storage;auth=main_window.auth
        smtp_config=auth.email_config
        if not smtp_config:
            QMessageBox.critical(self,"错误","未配置邮箱");return
        to_email=smtp_config.get('receiver_email')
        if not to_email:
            QMessageBox.critical(self,"错误","未设置收件邮箱");return
        entry=storage.get_entry_by_id(self.entry_id)
        if entry is None:
            QMessageBox.critical(self,"错误","未找到该文件记录");return
...
class UploadDialog(QDialog):
    ...
    def select_dest(self):
        ...
        if dir_:self.user_dest=dir_;self.dest_label.setText(dir_)
...
class LoginDialog(QDialog):
    ...
    def trigger_emergency_backup(self):
        ...
        if not file_info_list:
            QMessageBox.critical(self,"错误","没有可备份的文件");return
        try:
...
class SettingsDialog(QDialog):
    def __init__(self,parent,auth_manager,is_recovery_login=False):
        ...
        layout.addWidget(QLabel("当前已启用的验证方式："+status_text))
        ...
        # 主题
        th_layout=QHBoxLayout();th_layout.addWidget(QLabel("主题："))
        ...
        th_layout.addWidget(self.theme_combo);layout.addLayout(th_layout)
        # 迁移
        mg=QGroupBox("保险库迁移")
        ml=QVBoxLayout()
        self.btn_export_vault=QPushButton("导出保险库（备份/迁移）");self.btn_export_vault.clicked.connect(self.export_vault);ml.addWidget(self.btn_export_vault)
        self.btn_import_vault=QPushButton("导入保险库（恢复）");self.btn_import_vault.clicked.connect(self.import_vault);ml.addWidget(self.btn_import_vault)
        mg.setLayout(ml);layout.addWidget(mg)
        btn_box=QDialogButtonBox(QDialogButtonBox.Close);btn_box.rejected.connect(self.reject);layout.addWidget(btn_box)
        self.setLayout(layout)
    ...
    def _verify_identity(self):
        ...
        if not methods:
            QMessageBox.warning(self,"提示","没有可用的验证方式，无法修改。");return False
        d=DeleteAuthDialog(self,self.auth,methods)
    ...
    def generate_recovery(self):
        try:
            ...
            d=QDialog(self);d.setWindowTitle("紧急恢复代码")
            l=QVBoxLayout()
            l.addWidget(QLabel("您的紧急恢复代码已生成，请妥善保管："))
            cl=QLabel(code);cl.setStyleSheet("font-size:16pt;font-weight:bold;font-family:monospace;");cl.setAlignment(Qt.AlignCenter);l.addWidget(cl)
            ce=QLineEdit(code);ce.setReadOnly(True);ce.setStyleSheet("font-family:monospace;");l.addWidget(ce)
            eb=QPushButton("导出为 .txt 文件")
            def exp():
                try:
                    ...
                    if path:
                        ...
                        QMessageBox.information(d,"导出成功",f"代码已保存到：{path}")
                except Exception as e:QMessageBox.warning(d,"导出失败",str(e))
            eb.clicked.connect(exp);l.addWidget(eb)
            bb=QDialogButtonBox(QDialogButtonBox.Close);bb.rejected.connect(d.accept);l.addWidget(bb)
    ...
    def change_questions(self):
        if not self._verify_identity(): return
        d=QDialog(self);d.setWindowTitle("修改安全问题")
        l=QVBoxLayout()
        ...
        l.addWidget(QLabel("问题1"));l.addWidget(q1);l.addWidget(a1)
        l.addWidget(QLabel("问题2"));l.addWidget(q2);l.addWidget(a2)
        l.addWidget(QLabel("问题3"));l.addWidget(q3);l.addWidget(a3)
        bb=QDialogButtonBox(QDialogButtonBox.Ok|QDialogButtonBox.Cancel)
        bb.accepted.connect(d.accept);bb.rejected.connect(d.reject)
        l.addWidget(bb);d.setLayout(l)
    ...
    def change_totp(self):
        ...
        qr_data=self.auth.setup_totp()
        pixmap=QPixmap();pixmap.loadFromData(qr_data)
        d=QDialog(self);d.setWindowTitle("TOTP 设置")
        l=QVBoxLayout()
        l.addWidget(QLabel("请使用 Authenticator 扫描以下二维码："))
        il=QLabel();il.setPixmap(pixmap.scaled(200,200,Qt.KeepAspectRatio));l.addWidget(il)
        sl=QLabel(f"密钥：{self.auth.totp_secret}");l.addWidget(sl)
        ci=QLineEdit();ci.setPlaceholderText("输入动态码验证");l.addWidget(ci)
        bb=QDialogButtonBox(QDialogButtonBox.Ok|QDialogButtonBox.Cancel)
        bb.accepted.connect(d.accept);bb.rejected.connect(d.reject)
        l.addWidget(bb);d.setLayout(l)
        if d.exec_()==QDialog.Accepted:
            if not self.auth.verify_totp(ci.text()):
                QMessageBox.warning(self,"错误","验证码不正确");return
            QMessageBox.information(self,"成功","TOTP 已更新")

    def change_email(self):
        if not self._verify_identity(): return
        d=QDialog(self);d.setWindowTitle("修改邮箱配置")
        l=QVBoxLayout()
        ...
        l.addWidget(QLabel("SMTP服务器"));l.addWidget(smtp_server)
        l.addWidget(QLabel("端口"));l.addWidget(smtp_port)
        l.addWidget(QLabel("发件邮箱"));l.addWidget(sender_email)
        l.addWidget(QLabel("授权码/密码"));l.addWidget(sender_password)
        l.addWidget(QLabel("收件邮箱"));l.addWidget(receiver_email)
        bb=QDialogButtonBox(QDialogButtonBox.Ok|QDialogButtonBox.Cancel)
        bb.accepted.connect(d.accept);bb.rejected.connect(d.reject)
        l.addWidget(bb);d.setLayout(l)
        if d.exec_()!=QDialog.Accepted: return
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        l_layout.addWidget(QLabel("标签分类"))
        self.tag_list=QListWidget()
        self.tag_list.addItem("全部")
        self.tag_list.itemClicked.connect(self.on_tag_clicked)
        l_layout.addWidget(self.tag_list)
        tb_layout=QHBoxLayout()
        add_btn=QPushButton("+");add_btn.setToolTip("创建新标签");add_btn.clicked.connect(self.create_tag)
        del_btn=QPushButton("-");del_btn.setToolTip("删除选中标签");del_btn.clicked.connect(self.delete_tag)
        tb_layout.addWidget(add_btn);tb_layout.addWidget(del_btn)
        ...
        top_bar.addWidget(self.upload_btn);top_bar.addWidget(self.import_btn);top_bar.addWidget(self.export_btn)
        top_bar.addWidget(self.refresh_btn);top_bar.addWidget(self.settings_btn);top_bar.addWidget(self.delete_btn)
        ...
        r_layout.addWidget(self.file_list)
    ...
    def on_tag_clicked(self,item):
        if item.text()=="全部":self.current_tag=None
        else:self.current_tag=item.text()
        self.load_files()
    ...
    def delete_tag(self):
        ...
        tag=current.text()
        reply=QMessageBox.question(self,"确认",f"确定要删除标签 '{tag}' 吗？\n（文件本身不会被删除）",QMessageBox.Yes|QMessageBox.No)
    ...
    def _do_upload(self,file_path):
        d=UploadDialog(self,file_path)
        if d.exec_():
            try:
                uid=self.storage.add_file(file_path,d.user_dest,d.is_advanced,d.second_methods)
    ...
    def import_vault_file(self):
        path,_=QFileDialog.getOpenFileName(self,"选择要导入的 .vault 加密文件","","Vault Files (*.vault)")
        if not path:return
        is_advanced=QMessageBox.question(self,"高级文件","是否将此文件标记为高级文件（需二次验证）？",QMessageBox.Yes|QMessageBox.No)==QMessageBox.Yes
        second_methods=[]
        if is_advanced:
            d=QDialog(self);d.setWindowTitle("选择二次验证方式")
            l=QVBoxLayout()
            cb_totp=QCheckBox("TOTP");cb_email=QCheckBox("邮箱验证码");cb_question=QCheckBox("安全问题");cb_password=QCheckBox("密码")
            l.addWidget(cb_totp);l.addWidget(cb_email);l.addWidget(cb_question);l.addWidget(cb_password)
            bb=QDialogButtonBox(QDialogButtonBox.Ok|QDialogButtonBox.Cancel)
            bb.accepted.connect(d.accept);bb.rejected.connect(d.reject)
            l.addWidget(bb);d.setLayout(l)
            if d.exec_()==QDialog.Accepted:
                ...
                if not second_methods:
                    QMessageBox.warning(self,"提示","至少选择一种二次验证方式");return
        try:
            uid=self.storage.import_vault_file(path,is_advanced=is_advanced,second_auth_methods=second_methods)
    ...
    def export_decrypted_file(self):
        ...
        if not current:
            QMessageBox.warning(self,"提示","请先选择一个文件");return
        entry_id=current.data(Qt.UserRole)
        entry=self.storage.get_entry_by_id(entry_id)
        if entry is None:
            QMessageBox.warning(self,"错误","未找到该文件记录");return
        if entry['is_advanced']:
            methods=entry['second_auth_methods']
            if not methods:
                QMessageBox.warning(self,"提示","该高级文件未设置二次验证方式，无法导出");return
            d=AuthDialog(self,self.auth,methods,entry_id)
            if d.exec_()!=QDialog.Accepted:return
        else:
            methods=self._get_available_auth_methods()
            if not methods:
                QMessageBox.warning(self,"提示","没有可用的验证方式");return
            d=DeleteAuthDialog(self,self.auth,methods)
            if d.exec_()!=QDialog.Accepted:return
    ...
    def open_file(self,item):
        ...
        if entry['is_advanced']:
            methods=entry['second_auth_methods']
            if not methods:
                QMessageBox.warning(self,"提示","该文件未设置二次验证方式，无法打开");return
            d=AuthDialog(self,self.auth,methods,entry_id)
            if d.exec_()!=QDialog.Accepted:return
        try:
            data=self.storage.get_file_data(entry_id)
    ...
    def open_settings(self):
        d=SettingsDialog(self,self.auth,self.is_recovery_login)
        d.exec_()
        self.load_tags();self.load_files()
    ...
    def delete_file(self):
        ...
        d=DeleteAuthDialog(self,self.auth,methods)
        if d.exec_()!=QDialog.Accepted:return
        reply=QMessageBox.question(self,"确认删除","确定要永久删除该加密文件吗？\n（文件将被永久删除，无法恢复）",QMessageBox.Yes|QMessageBox.No)
        if reply==QMessageBox.Yes:
            try:
